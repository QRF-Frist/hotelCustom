<template>
	<div id="directCheckIn">
		<div class="customInfo">
			<p class="pTitle textColor textWeight">直接入住</p>
        ...
        <div class="cT_inputBox">
			...
			<div class="checkInfo_1 clearfix">
				<div class="item">
					...
					<div class="itemContent scrollBar" style="overflow-y: auto;height: 97%;">
						<el-radio-group v-model="chooseGender_radio">
				...
				<div class="item">
					...
					<div class="itemContent">
						<el-select v-model="identifierType" placeholder="请选择">
                            <el-option
                              v-for="item in identifierTypeOptions"
                              :key="item.value"
                              :label="item.label"
                              :value="item.value">
                            </el-option>
				...
				<div class="item">
					<span class="itemTitle">证件号</span>
					<div class="itemContent">
						<el-input
                          placeholder="请输入证件号"
                          v-model="inputIdentifierNum"
                          clearable>
                        </el-input>
					</div>
				</div>
            ...
            <div class="checkInfo_1 clearfix">
				...
				<div class="item">
					...
					<div class="itemContent">
						<el-select v-model="chooseNational" placeholder="请选择">
                            <el-option
	...
	export default {
		...
		data(){
			return {
               ...
               chooseGender_radio: '', // 入住人的性别
               inputCustomName: '', // 入住人的姓名
               identifierType: '', // 入住人的证件类型
               inputIdentifierNum: '', // 入住人的证件号
               inputPhoneNumber: '', // 入住人的手机号码
               chooseNational: '', // 入住人的所属民族
               operatorManName: '', // 当前订单操作员的名字
               orderNote: '', // 当前订单的备注
               ...
               orderSourceOptions:[{
                ...
                NationalArray:[ 
                   ...
                   { label: '瑶族', name: '瑶族' },
                   { label: '锡伯族', name: '锡伯族' },
                   { label: '乌孜别克族', name: '乌孜别族' },
                   { label: '维吾尔族', name: '维吾尔族族' },
                   { label: '佤族', name: '佤族' },
                   { label: '土家族', name: '土家族' },
                   { label: '土族', name: '土族' },
                   { label: '塔塔尔族', name: '塔塔尔族族' },
                   { label: '塔吉克族', name: '塔吉克族族' },
                   { label: '水族', name: '水族' },
                   { label: '畲族', name: '畲族' },
                   { label: '撒拉族', name: '撒拉族' },
                   { label: '羌族', name: '羌族' },
                   ...
                   { label: '黎族', name: '黎族' },
                   { label: '拉祜族', name: '拉祜族' },
                   { label: '柯尔克孜族', name: '柯尔克族' },
                   { label: '景颇族', name: '景颇族' },
                   { label: '京族', name: '京族' },
                   { label: '基诺族', name: '基诺族' },
                   { label: '回族', name: '回族' },
                   { label: '赫哲族', name: '赫哲族' },
                   { label: '哈萨克族', name: '哈萨克族族' },
                   { label: '哈尼族', name: '哈尼族' },
                   { label: '仡佬族', name: '仡佬族' },
                   { label: '高山族', name: '高山族' },
                   { label: '鄂温克族', name: '鄂温克族族' },
                   { label: '俄罗斯族', name: '俄罗斯族族' },
                   { label: '鄂伦春族', name: '鄂伦春族族' },
                   { label: '独龙族', name: '独龙族' },
                   { label: '东乡族', name: '东乡族' },
                   { label: '侗族', name: '侗族' },
                   { label: '德昂族', name: '德昂族' },
                   { label: '傣族', name: '傣族' },
                   { label: '达斡尔族', name: '达斡尔族族' },
                   { label: '朝鲜族', name: '朝鲜族' },
                   { label: '布依族', name: '布依族' },
                   { label: '保安族', name: '保安族' },
                   { label: '布朗族', name: '布朗族' },
...
<style>
#directCheckIn .el-radio {
   margin-top: 5px;
   margin-left: 20px;
}
#directCheckIn .el-radio__input.is-checked+.el-radio__label {
   color: rgba(76,109,232,.8);
}
#directCheckIn .el-radio__input.is-checked .el-radio__inner {
    border-color: rgba(76,109,232,.8);
    background: rgba(76,109,232,.8);
}
#directCheckIn .el-radio__inner:hover {
	border-color: rgba(76,109,232,.8);
}
#directCheckIn .cT_inputBox .el-input__inner {
	height: 25px;
	line-height: 25px;
}
#directCheckIn .el-input.is-active .el-input__inner,
#directCheckIn .el-input__inner:focus {
    border-color: rgba(76,109,232,.8);
}
#directCheckIn .customInfo .el-input-number--mini{
	width: 95%;
    line-height: 25px;
    float: right;
}
#directCheckIn .el-input-number__decrease:hover, 
#directCheckIn .el-input-number__increase:hover {
    color: rgba(76,109,232,.8);
}
#directCheckIn .el-input-number__decrease:hover:not(.is-disabled)~.el-input .el-input__inner:not(.is-disabled), .el-input-number__increase:hover:not(.is-disabled)~.el-input .el-input__inner:not(.is-disabled),
#directCheckIn .el-input-number__decrease:hover:not(.is-disabled)~.el-input .el-input__inner:not(.is-disabled), .el-input-number__increase:hover:not(.is-disabled)~.el-input .el-input__inner:not(.is-disabled)
{
   border-color: rgba(76,109,232,.8);
}
#directCheckIn .el-input-number__decrease:hover:not(.is-disabled)~.el-input .el-input__inner:not(.is-disabled),
#directCheckIn .el-input-number__increase:hover:not(.is-disabled)~.el-input .el-input__inner:not(.is-disabled)
 {
   border-color: rgba(76,109,232,.8);
 }
#directCheckIn .checkInfo_1 .el-date-editor.el-input,
#directCheckIn .checkInfo_1 .el-date-editor.el-input__inner {
   width: 85%;
 }
#directCheckIn .checkInfo_1 .el-input__inner {
 	height: 30px;
 	line-height: 30px;
 }
#directCheckIn .checkInfo_1 .el-input__icon {
 	line-height: 30px;
 }
 #directCheckIn .itemContent .el-radio {
    margin-left: 5px;
    margin-right: 10px;
 }
#directCheckIn .el-checkbox__inner:hover,
#directCheckIn .el-checkbox__inner:focus
 {
    border-color: rgba(76,109,232,.8);
}
#directCheckIn .el-checkbox__input.is-checked+.el-checkbox__label 
{
	color: rgba(76,109,232,.8);
}
#directCheckIn .el-checkbox__input.is-checked .el-checkbox__inner, 
#directCheckIn .el-checkbox__input.is-indeterminate .el-checkbox__inner {
    background-color: rgba(76,109,232,.8);
    border-color: rgba(76,109,232,.8);
}
#directCheckIn .el-button--primary,
#directCheckIn .el-button--primary:focus,
#directCheckIn .el-button--primary:hover {
    background: rgba(76,109,232,.8);
    border-color: rgba(76,109,232,.8);
    color: #FFF;
}
